...
internal static class RtsSampleSettingsStore
{
    ...
    static RtsSampleSettingsStore()
    {
        JsonOptions.Converters.Add(new GraphicsBackendJsonConverter());
        JsonOptions.Converters.Add(new JsonStringEnumConverter(JsonNamingPolicy.CamelCase));
    }

    public static RtsSampleSettings Load(string path)
    {
        if (File.Exists(path))
        {
            try
            {
                ...
                var settings = JsonSerializer.Deserialize<RtsSampleSettings>(json, JsonOptions);
        ...
        var defaults = new RtsSampleSettings();
        Save(path, defaults);
        return defaults;
    }

    public static void Save(string path, RtsSampleSettings settings)
    {
        settings.Sanitize();
